...
dt = 0.01;              % Timestep of integration. Each substep lasts this long.
success = false;        
bonus = 0;
...
CostHistory = zeros(1,maxep);  % To save cost for NN for each episode
...
for episodes = 1:maxep
    ...
    % Save Cost
    J = norm(CostFunction(NNoutput, NNpred));
    %disp(size(J))
    CostHistory(episodes) = J;
...
%-- Q-learning --%
tSteps = 600; 		% Set number of iterations because there are undefined states
gamma = 0.3; 		% Discount rate
ballRadius = pi/2-0.9;
fball = @(x) 5*tan(abs(pi/ballRadius*x-3*pi/2));
fballabs = @(q) min(fball(q)*(q<ballRadius/2),15) + max((q>=ballRadius/2)* fball( min(q,ballRadius-0.01) ),-15);        %(1+sign(fball(q)))/2 ;%+ max(fball(min(q,ballRadius-0.0001)) ,-15)*(1-sign(fball(q)))/2;  min(fball(q),15)*
Rfunc = @(q) fballabs(abs(q-pi));
%Rfunc = @(q) min(1/abs(q-pi),15); %-2*abs(q-pi)^1.3;  < 0) || (obs(1) >
epsilonDecay = 0.999;
lrate = 0.2;
...
Neurons = [InputSize 64 32 OutputSize];         % Neurons per layers (input, hidden1, hidden2, output)
...
while(~cFlag)
    ...
    while(episodes<maxep+1)
        ...
        for iter = 1:tSteps
            ...
            r = Rfunc(obs(1)) - 2*abs(obs(2))^1.1;
            RewardHistory(iter, episodes) = r;

        	% Save input and output for backpropagation
            NNinput(iter,:) = state;

            if ( (obs(1) > 3*pi/2) || (obs(1) < pi/2)  ) || ( abs(obs(2)) > 8 )
...
%           break;
    ...
    disp(['Max iter ', num2str(max(stats)),' Last Iter ', num2str(stats(end)), ' Mean Iter ', num2str(mean(stats))]);
    %cFlag = true;
end % While Loop
...
%%
% Ploting phase portrait
xd_sp = linspace(-4,4,50);
x_sp = linspace(pi/2,3*pi/2,50);
[x_grid, xd_grid] = meshgrid(x_sp,xd_sp);
xdnT_grid = zeros(size(x_grid));
xd0T_grid = zeros(size(x_grid));
xdpT_grid = zeros(size(x_grid));
...
for i = 1:size(xd_grid,1)*size(xd_grid,2)
    state_temp = [x_grid(i) , xd_grid(i)];
    [q_temp , ~, ~, ~, ~, ~]= Feedforward(state_temp, W1, W2, W3, ActFuncType);
    [~,ActIndex_temp] = max(q_temp);
    stated_temp = Dynamics(state_temp,actions(ActIndex_temp));
    xddnT_grid(i) = stated_temp(2)*(ActIndex_temp==1);
    xdd0T_grid(i) = stated_temp(2)*(ActIndex_temp==2);
...
simulate1Pend(parameters, simS,dt);
