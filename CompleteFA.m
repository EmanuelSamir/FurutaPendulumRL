...
for num = 1:length(bestA)
    ...
    for i = 1:2
        ...
        obs = lastState + dt/6*(k1 + 2*k2 + 2*k3 + k4);
        % All states wrapped to 2pi
        if (obs(1) < 0) || (obs(1) > 2*pi)
            obs(1) = normalAngle(obs(1));
        end
...
for num = 1:length(bestA)
    ...
    for i = 1:2
        ...
        obs = finalState + dt/6*(k1 + 2*k2 + 2*k3 + k4);
        % All states wrapped to 2pi
        if (obs(1) < 0) || (obs(1) > 2*pi)
            obs(1) = normalAngle(obs(1));
        end
...
% Listings en latex
% Grabar todas las acciones e integrar al final
% grafica de estado vs tiempo (angulo y velocidad)
% Si alcanza el espacio, las acciones vs tiempo